...
foreach demo in gender so {
    ...
    * Model 3 (SO only): add assigned sex at birth
    if "`demo'" == "so" {
        logit segment b0.`demo'_cat `controls' i.afab
        estimates store segment_`demo'3
        estadd local demo_controls "Yes"
        estadd local afab_control "Yes"

        esttab segment_`demo'1 segment_`demo'2 segment_`demo'3 using $csacprojdir/tab/thsj_rr/segment_models_`demo'.rtf, ///
            keep(*`demo'_cat*) ///
            nobaselevels ///
            b(%9.3f) se(%9.3f) ///
            star(* 0.10 ** 0.05 *** 0.01) ///
...
foreach demo in gender so {
    ...
    * Model 3 (SO only): demographics + assigned sex at birth
    if "`demo'" == "so" {
        ...
        esttab deg_`demo'1 deg_`demo'2 deg_`demo'3 using $csacprojdir/tab/thsj_rr/degree_`demo'.rtf, ///
            keep(*`demo'_cat*) ///
            nobaselevels ///
            b(%9.3f) se(%9.3f) ///
            star(* 0.10 ** 0.05 *** 0.01) ///
    ...
    *===============================================================================
    * DEGREE COMPLETION collapsed category
    *===============================================================================
    * Model 1: unconditional
    ...
    * Model 2: with demographic controls
    ...
    if "`demo'" == "so" {
        ologit highest_degree_2 i.`demo'_cat `controls' i.afab
        estimates store deg2_`demo'3
        estadd local demo_controls "Yes"
        estadd local afab_control "Yes"

        esttab deg2_`demo'1 deg2_`demo'2 deg2_`demo'3 using $csacprojdir/tab/thsj_rr/degree2_`demo'.rtf, ///
            keep(*`demo'_cat*) ///
            nobaselevels ///
            b(%9.3f) se(%9.3f) ///
            star(* 0.10 ** 0.05 *** 0.01) ///
    ...
    *===========================================================================
    ...
    if "`demo'" == "so" {
        * Regression tables with odds ratios — 3 columns for SO
        esttab deg_`demo'1 deg_`demo'2 deg_`demo'3 using $csacprojdir/tab/getting_down_to_facts/degree_or_`demo'.rtf, ///
            keep(*`demo'_cat*) ///
            nobaselevels ///
            eform ///
            b(%9.3f) ci(%9.3f) ///
        ...
        esttab deg_`demo'1 deg_`demo'2 deg_`demo'3 using $csacprojdir/tab/getting_down_to_facts/degree_or_`demo'.csv, ///
            keep(*`demo'_cat*) ///
            nobaselevels ///
            eform ///
            b(%9.3f) ci(%9.3f) ///
        ...
        * Segment logit as odds ratios — 3 columns for SO
        esttab segment_`demo'1 segment_`demo'2 segment_`demo'3 using $csacprojdir/tab/getting_down_to_facts/segment_or_`demo'.rtf, ///
            keep(*`demo'_cat*) ///
            nobaselevels ///
            eform ///
            b(%9.3f) ci(%9.3f) ///
    ...
    *===========================================================================
    * STACKED BAR WITH AFAB CONTROL (SO only)
    *===========================================================================
    if "`demo'" == "so" {

        * 5-category degree with afab control
        ologit highest_degree i.`demo'_cat `controls' i.afab
        ...
        export delimited group_lab pr_cert pr_assoc pr_bach pr_mast pr_doct ///
            using $csacprojdir/tab/getting_down_to_facts/degree_margins_so_afab.csv, replace

        restore

        * 4-category degree with afab control
        ologit highest_degree_2 i.`demo'_cat `controls' i.afab
        ...
        export delimited group_lab pr_cert pr_assoc pr_bach pr_mastdoct ///
            using $csacprojdir/tab/getting_down_to_facts/degree2_margins_so_afab.csv, replace
